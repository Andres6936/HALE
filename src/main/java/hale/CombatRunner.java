...
public class CombatRunner
{
    ...
    public boolean provokeAttacksOfOpportunity(Creature target, MovementHandler.Mover mover)
    {
        ...
        if (mover == null) {
            threateningCreatures = getThreateningCreatures(target);
        } else {
            threateningCreatures = getThreateningCreaturesAtNextPosition(target);
        }
        ...
        for (Creature current : threateningCreatures) {
            ...
            if (current instanceof PC) {
                ...
                if (current.getTemplate().hasScript()) {
    ...
    private void runTurn()
    {
        ...
        while (continueCombat && !isPartyDefeated()) {
            ...
            if (current instanceof PC) {
                ...
                if (current.getTemplate().hasScript() && current.isAIActive()) {
